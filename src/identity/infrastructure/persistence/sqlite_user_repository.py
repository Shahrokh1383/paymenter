...
class SqliteUserRepository(UserRepository):
    ...
    def get_all_summaries(self) -> List[Any]:
        return self._uow.conn.execute("""
            SELECT u.id, u.name, u.phone_email, a.id as account_id, a.account_number, uc.card_number, a.balance, c.code as currency_code
            FROM users u 
    ...
    def search_summaries(self, query: str) -> List[Any]:
        ...
        return self._uow.conn.execute("""
            ...
            LEFT JOIN user_cards uc ON a.id = uc.account_id
            LEFT JOIN currencies c ON a.currency_id = c.id
            WHERE u.name LIKE ? OR a.account_number LIKE ? OR uc.card_number LIKE ?
